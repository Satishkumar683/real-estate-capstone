...
PROJECT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', '..', 'data', 'raw')
...
def scrape_flats_for_city(driver, city: str, start: int, end: int, project_dir: str) -> pd.DataFrame:
    page_number = start
    flats = pd.DataFrame()
    ...
    try:
        while page_number < end:
            ...
            for item in listing_data:
                ...
                random_delay()
                dpage_soup = BeautifulSoup(driver.page_source, 'html.parser')
...
if __name__ == '__main__':
    ...
    try:
        for city in CITIES:
            ...
            scrape_flats_for_city(driver, city, start_page, end_page, PROJECT_DIR)
    finally:
        driver.quit()