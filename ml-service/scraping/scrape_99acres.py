# -*- coding: utf-8 -*-
"""
99acres Flats/Apartments Scraper - Selenium version (Multi-City)

Built on Selenium instead of requests, since 99acres blocks plain HTTP
requests via bot-fingerprint detection but allows a real (even headless)
browser through.

CHANGELOG (this version):
- Added latitude/longitude extraction from the JSON-LD <script type="application/ld+json">
  block on the property detail page (data['geo']['latitude'/'longitude']).
  NOTE: 99acres frequently stores these two fields SWAPPED (their "latitude"
  is actually the longitude and vice versa). This script detects that using
  India's rough coordinate bounding box (lat ~6-37, lon ~68-97) and corrects it.
- Added societyName + main image URL, also pulled from JSON-LD (more reliable
  than scraping the DOM for these).
- Added locality breadcrumb hierarchy from the BreadcrumbList JSON-LD block.
- Added RERA status + RERA website.
- Added "Overlooking" field.
- Replaced the old hardcoded #Transact_Type_Label / #Owntype_Label / etc.
  selectors with a generic parser over #AdditionalDetailsComponent that reads
  whatever label/value pairs are present (transaction type, ownership,
  flooring, width of facing road, parking, power backup, etc.) - this way it
  won't silently return blank if 99acres reorders or adds/removes rows.
- Added dealer/agent details: name, company, member-since, address, website.
- Added posted date and price-per-sqft.
- Fixed property_id: the old `#Prop_Id` selector does not exist on the
  current page template (verified against a real page - it just returns
  blank every time). Now pulled via regex from the `spid-XXXXXXX` in the URL,
  which is always present and reliable.

*** THIS VERSION: adds Webshare proxy support (see "PROXY CONFIG" section
    below). Chrome's --proxy-server flag can't carry a username/password,
    so authenticated proxies (which is what Webshare's free tier gives you)
    are injected via a tiny auto-generated Chrome extension instead.
    Proxies are ROTATED round-robin through PROXY_POOL every
    PROXY_ROTATE_EVERY requests - since Selenium can't swap a proxy on a
    live browser, rotation means quitting the current Chrome instance and
    launching a fresh one on the next proxy in the pool. ***
"""
